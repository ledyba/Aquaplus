...
static int PAC_DecodeMemoryBmpTbl( MOVE_INFO *mi, void *dest, int draw_mode, int bai, DWORD now_esize )
{
	...
	DWORD		dsize;
	RGB32		pal[256];
	BYTE		*src = mi->buf+sizeof(RGB32)*256;
	int			i;
	...
	if( now_esize==(DWORD)(mi->bmi.sx*mi->bmi.sy+1024) ){
		...
		switch( draw_mode ){
			...
			case 24:
				...
				dsize = LZS_DecodeMemoryBmpTbl_3BYTE( (BYTE*)bmp_f->buf+buf_point, mi->buf, now_esize, mi->bmi.sabun, bai, mi->r, mi->g, mi->b );
...
BOOL PAC_PackFileMovie_NextRead( MOVE_INFO *mi, void *dest, int draw_mode, int bai, int count )
{
	int			i;
	DWORD		esize;
	DWORD		now_esize;
...
BOOL PAC_PackFileMovie_Read( int move_no, void *dest, int draw_mode, int bai, int frame )
{
	MOVE_INFO	*mi = &MoveInfo[move_no];
	int			now_time, plan_time;
	int			sleep_time, over_frame=0;

	if( mi->fh ){
		if(mi->cond==MOVE_COND_PLAY){
			now_time   = STD_timeGetTime();									
			plan_time  = mi->start_time + mi->count*1000/mi->bmi.frame;	
			sleep_time = plan_time-now_time;							
			if(sleep_time<0)
				over_frame = -sleep_time*mi->bmi.frame/1000;	
			...
			if( !PAC_PackFileMovie_NextRead( mi, dest, draw_mode, bai, over_frame+1) ){	
				switch(mi->loop){
					case 0:	PAC_PackFileMovie_Start( move_no, MOVE_COND_PLAY, ON );	break;
					case 1:	PAC_PackFileMovie_Start( move_no, MOVE_COND_END, OFF );	break;
					default:PAC_PackFileMovie_Start( move_no, MOVE_COND_PLAY, --mi->loop );	break;
				}
			}
		}
	}
...
static	BYTE	text_buf[RBUF_SIZE+UPPER_LIMIT-1];


int  LZS_DecodeMemoryBmpTbl_DWORD( RGB32 *dst, BYTE *src, int file_size, int sabun, int bai, int rr, int gg, int bb )
{
	int		j, r, c;
	DWORD	flags = 0;
	RGB32	*dst2;
...
int  LZS_DecodeMemoryBmpTbl_3BYTE( BYTE *dst, BYTE *src, int file_size, int sabun, int bai, int rr, int gg, int bb )
{
	int		j, r, c;
	DWORD	flags = 0;
	BYTE	*dst2;
...
int  LZS_DecodeMemoryBmpTbl_BYTE( BYTE *dst, RGB32 *tbl, BYTE *src, int file_size, int sabun )
{
	int		j, r, c;
	DWORD	flags = 0;
	BYTE	*dst2;
