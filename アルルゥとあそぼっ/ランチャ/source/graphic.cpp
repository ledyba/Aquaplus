...
BOOL ColorBuf::BltFast(int xDest,int yDest,ColorBuf *srcCBuf,RECT *srcRect,BOOL bAlpha)
{
	...
	if(bAlpha && srcCBuf->alpha==0)return TRUE;
	...
	if(NULL==srcRect){
		...
		if(widthSrc<=0 || heightSrc<=0)return TRUE;
	}else{
		xSrc = srcRect->left;
		ySrc = srcRect->top;
		if(xDest < 0){
		...
		widthSrc = srcRect->right -xSrc;
		heightSrc = srcRect->bottom -ySrc;
		if(widthSrc<=0 || heightSrc<=0)return TRUE;
	}
...
BOOL c_disp(int char_no, int pose, int locate, int in_type, int layer,int frame)
{
	...
	for(i=0;i<STCHAR_MAX;i++){
		if(charInf[i].save.char_no==char_no){
			bNoChangeLayer = TRUE;
			charInf[i].out_type = _CHR_DIRECT;
			if(_CHR_FADE==charInf[i].in_type){
				if(charInf[i].look_cnt<=timeGetTime() || G_SKIP){
					charInf[i].look_cnt = 0;
					charInf[i].in_type = _CHR_DIRECT;
...
charLoad:
	...
	charInf[i].colorBuf.offset.x = 80;
	charInf[i].colorBuf.offset.y = 30+(600-charInf[i].colorBuf.height);

	switch(locate){
	  case _CHR_LL:
		charInf[i].colorBuf.offset.x += (-280 +40);
		break;
	  case _CHR_RR:
		charInf[i].colorBuf.offset.x += (280 +40);
		break;
	  case _CHR_CL:
		charInf[i].colorBuf.offset.x += (-80 +40);
		break;
	  case _CHR_CR:
		charInf[i].colorBuf.offset.x += (80 +40);
		break;
...
void SetBmpInf::SetBmpZoom(int x, int y, int width, int height)
{
	...
	srcRect.top = 0;
	if(y<0){
		srcRect.top = ((-y)/(float)height)*colorBuf.height;
		dstRect.top = 30;
	}
	if((y+height)>WIN_SIZEY){
		srcRect.bottom = (1.0f -((y+height)-WIN_SIZEY)/(float)height)*colorBuf.height;
		dstRect.bottom = WIN_SIZEY +30;
	}
	srcRect.left = 0;
	if(x<0){
		srcRect.left = ((-x)/(float)width)*colorBuf.width;
		dstRect.left = 40;
	}
	if((x+width)>WIN_SIZEX){
		srcRect.right = (1.0f -((x+width)-WIN_SIZEX)/(float)width)*colorBuf.width;
...
void D3lib_LvertexToTLvertex(D3DXVECTOR3 *vt, D3DXMATRIX *cWorld)
{
	D3DXVECTOR4 test;
	D3DXVec3Transform(&test,vt,cWorld);

	vt->x = 400*(test.x/test.w) +400;
	vt->y = -300*(test.y/test.w) +300;
	return;
	...
	D3DXVECTOR3 a;
	float cmZ = (cWorld->_14*vt->x + cWorld->_24*vt->y + cWorld->_34*vt->z + cWorld->_44);

	float rhw = 1.0f / cmZ;
	a.x =  (cWorld->_11*vt->x + cWorld->_21*vt->y + cWorld->_31*vt->z + cWorld->_41) *rhw;
	a.y =  (cWorld->_12*vt->x + cWorld->_22*vt->y + cWorld->_32*vt->z + cWorld->_42) *rhw;
	vt->z = (cWorld->_13*vt->x + cWorld->_23*vt->y + cWorld->_33*vt->z + cWorld->_43) *rhw;
...
D3DXMATRIX matWorld;
D3DXMATRIX matView, matProj,matProjView;
...
void InitMaze()
{
	...
	D3DXMatrixMultiply(&matProjView,&matView,&matProj);
};
...
void GetMazeVec(D3DXVECTOR3 *vec, float deg)
{
    ...
    D3DXMatrixRotationY( &matWorldSpin, DEGtoRAD(deg) );

	D3DXMatrixMultiply(&matWorld,&matWorldSpin,&matProjView);

	D3lib_LvertexToTLvertex(vec,&matWorld);
}