...
BOOL ClMsgWnd::InitMessageParts()
{
	...
	MsgCLS();
	ForceHide();
	msgPlate.offset.x = 14;	msgPlate.offset.y = 38;
	return TRUE;
...
BOOL ClMsgWnd::BltMessage()
{
	...
	if(namePlate.alpha){
		...
		g_DibInf.colorBuf.BltFast(offset.x+44, offset.y+15, &namePlate,NULL,bAlphaUse);
	}
	...
	g_DibInf.colorBuf.BltFast(offset.x+616, offset.y+78, &msgWait, &rect, TRUE);
	return TRUE;
} // ClMsgWnd::BltMessage
...
void ClMsgWnd::MsgTypeChar(BYTE *msgBuf)
{
	...
	if(*msgBuf >= 0x80){
		...
		msgWidth ++;
		widthOffset += (sysInf.FontSize/2 +1);
			  ...
			  case '<':
				switch(*(msgBuf+1)){
				  case 'R':  case 'r':
					...
					rubiOffset.y = heightOffset-(rubiHeight+1);
				  ...
				  case '@':
					msgBuf += 2;
					OutputOutlineFontWidth(&msgPlate,(BYTE *)msgBuf ,widthOffset,heightOffset,sysInf.FontSize,2);
					msgBuf += 3;
					msgWidth += 4;
					widthOffset += (sysInf.FontSize*2);
					break;
...
void OutputOutlineFont(ColorBuf *destCBuf,BYTE *str,int mx,int my,int size,myRGB *color)
{
	...
	for(y=0;y<orgSize;y++){
		lpPixa = (myRGBA *)(destCBuf->pBuf +destCBuf->wPitch*(my+y)) +mx;
...
#ifndef bKage
				if(c[i]){
					lpPixa[oX].bAlpha = (c[i])*0x80/15 +0x7f;
					if(color==NULL){
						lpPixa[oX].bRed = lpPixa[oX].bGreen = lpPixa[oX].bBlue = 0xff;
					}else{
						lpPixa[oX].bRed   = color->bRed;
						lpPixa[oX].bGreen = color->bGreen;
						lpPixa[oX].bBlue  = color->bBlue;
					}
				}
#else
				if(c[i]>=6){
					if(size!=systemFontSize){
						lpPixa[oX].bAlpha = 0xff;
						lpPixa[oX].bRed = lpPixa[oX].bGreen = lpPixa[oX].bBlue = (c[i]-5)*0x7f/10 +0x7f;
					}else{
						lpPixa[oX].bAlpha = (c[i]-5)*0xdf/10 +0x20;
						lpPixa[oX].bRed = lpPixa[oX].bGreen = lpPixa[oX].bBlue = 0xff;
					}
				}else if(c[i]){
					if(c[i]==5) lpPixa[oX].bAlpha = 0xff;
					else lpPixa[oX].bAlpha = c[i]*0xff/5;
					lpPixa[oX].bRed = lpPixa[oX].bGreen = lpPixa[oX].bBlue = 0;
				}
#endif
			}
		}
		...
		for(x=0;x<orgSize/2;x++){
			...
			for(i=0;i<2;i++){
				...
				if(c[i]){
					c[i] = 17*c[i];
					for(oY=sY;oY<eY;oY++){
						lpPixa = (myRGBA *)(destCBuf->pBuf +destCBuf->wPitch*(my+oY)) +mx;
						for(oX=sX;oX<eX;oX++){
							lpPixa[oX].bAlpha = c[i];
							lpPixa[oX].bRed = lpPixa[oX].bGreen = lpPixa[oX].bBlue = 0xff;
						}
					}
				}
...
void OutputOutlineFontWidth(ColorBuf *destCBuf,BYTE *str,int mx,int my,int size,int expansion)
{
	...
	if(my<0)my = 0;
	offset = (96+189*42)*orgSize*orgSize/2;
	if(*(str+1)==0x60) offset += (orgSize*orgSize);
...
#ifndef bKage
				if(c[i]){
					lpPix[oX].bAlpha = (c[i])*0x90/15 +0x6f;
