...
void BackLog::Exit()
{
	...
	sideWnd.btn[0].SetState(btn_normal);
	sideWnd.btn[1].SetState(btn_disable);
	...
	if(keyState.push_cancel){
		...
		for(i=dispTop;(i!=logBottom && line>0);i++){
			...
			if(logVoice[i][0]!='\0'){
				...
				voiceRect.top = (15-line)*(mainFontSize+rubiHeight)+5;
...
void BackLog::SetBackLog(char *msg, int line, int bPage)
{
	int		len;
	WORD	oldOffset;
	...
	if(FALSE==bMsgContinue){
		strcpy(&logWork[oldOffset],msg);
		logLine[logBottom] = line;
	}else{
		if( oldOffset+strlen(&logWork[oldOffset])+len >= LOG_SIZE ){	
			strcpy(logWork, &logWork[oldOffset]);
			logWork[oldOffset] = '\0';
			oldOffset = 0;
			logOffset[logBottom] = 0;
		} 
		strcat(&logWork[oldOffset],msg);
		logLine[logBottom] += (line -1);
	}
	if(bPage==2){
		logBottom ++;
		bMsgContinue = FALSE;
	...
	if(bPage==2){
		logOffset[logBottom] = oldOffset +strlen(&logWork[oldOffset]) +1;
	}
...
void BackLog::DispInit()
{
	...
	voiceMark.loadLGF(pack_gparts,"voiceMark");
	...
	sideWnd.btn[1].SetState(btn_normal);
} // BackLog::DispInit
...
BOOL BackLog::TextDraw()
{
	...
	while(tmp_top!=logBottom){
		if(logVoice[tmp_top][0] != '\0'){
			msgPlate.BltFast(4,msgHeight*(mainFontSize+rubiHeight)+rubiHeight,&voiceMark,NULL,FALSE);
		}
		DisplayMessage((BYTE *)&logWork[logOffset[tmp_top]]);
		if(msgHeight>=15)break;
		tmp_top ++;
		if(tmp_top > 99)tmp_top = 0;
	}
	dispLogLine = 0;
	...
	if(logTop==dispTop && 0==blockOffset){
		sideWnd.btn[0].SetState(btn_disable);
	}else{
		sideWnd.btn[0].SetState(btn_normal);
	}
	bSelectVoice = FALSE;
...
void BackLog::MsgTypeChar(BYTE *msgBuf)
{
	...
	if(0==tmpBlockOffset){
		...
		OutputOutlineFont(&msgPlate,(BYTE *)msgBuf ,msgWidth*(mainFontSize/2)+mainFontSize,msgHeight*(mainFontSize+rubiHeight)+rubiHeight,mainFontSize);
	}
	if(*msgBuf >= 0x80){
		msgWidth += 2;
	}else{
		...
		if(*msgBuf){
			switch(*msgBuf){
			  ...
			  case '<':
				switch(*(msgBuf+1)){
				  case 'R':  case 'r':	
					rubiOffset.x = msgWidth*(mainFontSize/2)+mainFontSize;
