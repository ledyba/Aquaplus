...
public:
	...
	ClResult ChangeMusic(int &handle,int arcFileNum,int num,BOOL bFade,int repeat);
	ClResult playVoice(int &handle,int arcFileNum,char *fname,BOOL bLoop=FALSE);
	ClResult play(int handle,int repeat);
	void NextMusicStart(void);
	void Stop(int handle);
	ClResult FadeOut(int handle,int frame=25);
...
#define	SelectSnd		9014		
#define CancelSnd		9002		
#define CurMoveSnd		2210		
...
inline void PlayEffect(int x,BYTE vol,BOOL bLoop=FALSE)
{
	int	loopCnt = 1;
	if(bLoop)loopCnt = 0;
	lpSoundDS->SetVolume(x,vol*sysInf.seVolume/10);
	lpSoundDS->play(x,loopCnt);
}

inline void PlayEffectNum(int &x,int y,int vol=255,BOOL bLoop=FALSE)
{
	char	fn[32];
