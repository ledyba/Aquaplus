...
#define CG_POS		(DATE_MAX*SCRIPT_MAX*BLOCK_MAX/8)



void AVG_SetCgFlag(int cg_no )
{
	DWORD	fSize;
	set1BitFlag(ScenarioFlag[SCRIPT_MAX-2], cg_no, on);
	SetFilePointer(sysInf.configHF, (SCRIPT_MAX-2)*BLOCK_MAX/8, NULL, FILE_BEGIN);
	WriteFile(sysInf.configHF,ScenarioFlag[SCRIPT_MAX-2],BLOCK_MAX/8,&fSize,NULL);
}




BOOL AVG_GetCgFlag( int cg_no )
{
	return look1BitFlag( ScenarioFlag[SCRIPT_MAX-2], cg_no );
}
...
static void ESC_EOprSetMessage2(void)
{
	...
	if(NULL==msgBuf){
		switch(EOprFlag[ESC_SETMESSAGE2]){
		  case 1:
			backLog.SetBackLog(EscParam[0].str, msgWnd.logMsgLine, EscParam[1].num);
...
static void ESC_EOprV( void )
{
	...
	if(EOprFlag[ESC_V]==0 && _BAK_DIRECT!=EscParam[0].num){
		...
		sysInf.bForceRender = on;
		AVG_SetCgFlag( 0 );
		AVG_SetCgFlag( EscParam[1].num +1 );
	}else{
...
{
	if(NULL==lpStaffRoll){
		lpStaffRoll = new StaffRoll( EscParam[0].num );
	}
