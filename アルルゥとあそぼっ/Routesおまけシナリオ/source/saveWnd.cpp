...
void SAV_LoadScript(void)
{
	...
	*EXEC_LangInfo      = SaveStruct.EXEC_LangInfo;
	...
	_splitpath(NowLangFileName,NULL,NULL,str,NULL);
...
BOOL SAV_Load( int load_no )
{
	...
	if(saveInf.selectNum){
		...
		backLog.SetBackLog(backSelectInf.msgBuf,backSelectInf.msgLine,2);
		sysInf.bMsgCLS = on;
	}
...
SaveWnd::SaveWnd(exec_mode s_mode)
{
	...
	overWriteWnd.loadLGF(pack_gparts,"sys0136");
	overWriteBtn.loadLGF(pack_gparts,"sys0138");
	...
	rect.left = 0 +260;	rect.right = rect.left +130;
	rect.top = 230 +90;	rect.bottom = rect.top +24;
	for(i=0;i<3;i++){
		ptRect[i].left = 0;		ptRect[i].right = ptRect[i].left +130;
		ptRect[i].top = i*24;	ptRect[i].bottom = ptRect[i].top +24;
	}
	ov_btn[0].Create(1,&overWriteBtn,&rect,ptRect,&g_DibInf.colorBuf);
	rect.left = 0 +405;	rect.right = rect.left +130;
	rect.top = 230 +90;	rect.bottom = rect.top +24;
	for(i=0;i<3;i++){
		ptRect[i].left = 130;		ptRect[i].right = ptRect[i].left +130;
		ptRect[i].top = i*24;	ptRect[i].bottom = ptRect[i].top +24;
	}
	ov_btn[1].Create(1,&overWriteBtn,&rect,ptRect,&g_DibInf.colorBuf);
	time_t		tmp_time = 0;
	for(i=0;i<50;i++){
		char	fname[32];
...
void SaveWnd::overWriteExec()
{
	int		i;
	for(i=0;i<2;i++){
		if(ov_btn[i].CheckState(&sysInf.msPoint)){
			switch(i){
			  case 0:	
				PlayEffectNum(evtHandle[1],SelectSnd);
		...
		if(selectNum<10){
			if(mode==save_mode || useTable[selectNum+offsetNum]){
				if(mode==save_mode && useTable[selectNum+offsetNum]){
					PlayEffectNum(evtHandle[1],SelectSnd);
					mode = overwrite_mode;
				}else if(mode==save_mode || mode==overwrite_mode){
					PlayEffectNum(evtHandle[1],SelectSnd);
					new_time = selectNum+offsetNum+1;
					SAV_Save( new_time );
					useTable[selectNum+offsetNum] = TRUE;
...
void SaveWnd::Blt()
{
	...
	if(save_mode==mode || overwrite_mode==mode)rect.top = 36;
	...
	if(overwrite_mode==mode){
		g_DibInf.colorBuf.BltFast(0,230,&overWriteWnd,NULL,TRUE);
		for(i=0;i<2;i++){
			ov_btn[i].Draw();
		}
	}
