...
ClSoundDS::ClSoundDS(
	HWND hWnd,
	ClReadFile *readFile,
	BYTE p_bgm, BYTE p_sound, BYTE p_voice,
	BYTE v_bgm, BYTE v_sound, BYTE v_voice,
	BOOL b3DSound)
{
	int				i;
	HRESULT			hr;
	...
	for(i=0;i<hEvtSndMax;i++){ evtHandle[i] = -1; }	
    lpReadFile = readFile;
	Pack_bgmfile = p_bgm;	Pack_sound = p_sound;	Pack_voice = p_voice;
	bgmVolume = v_bgm;		seVolume = v_sound;		voiceVolume = v_voice;
...
ClResult ClSoundDS::CreateSoundObj(int &handle,int arcFileNum,int playNum,BOOL bStream,BOOL b3DSound)
{
	...
	if(FALSE==bStream){
		...
		ret = AddSoundBuffer(handle,fileBuf,playNum,b3DSound);
		cl_free(fileBuf);
	}
...
ClResult ClSoundDS::ChangeMusic(int &handle,int fnum,BOOL bFade,int repeat,int fo_frame, int volume)
{
	BOOL			ret;
	ClSoundBuffer	*lpSoundBuffer;
	int				num;
	...
	if(0==fo_frame)bFade = FALSE;
	musicNum = fnum;
	musicVol = volume;
	bChangeingMusic = TRUE;
	...
	if(lpSoundBuffer){
		...
		if(status != PCM_STOP){
			if(lpSoundBuffer->orgMusicNum==num && status!=PCM_FADEOUT){
				if(volume != lpSoundBuffer->volume)	SetVolume(handle, volume, bgmVolume);
				goto SetMusicInfo;
			}
			if(FALSE==bFade){
				Stop(handle);
				...
				if(PCM_FADEOUT==status){
					float f_volume = lpSoundBuffer->fo_value
									+float(lpSoundBuffer->volume -lpSoundBuffer->fo_value) *lpSoundBuffer->fo_step /lpSoundBuffer->fo_frame;
					lpSoundBuffer->volume = int(f_volume);
					lpSoundBuffer->fo_frame = fo_frame;
					lpSoundBuffer->fo_step = lpSoundBuffer->fo_frame -1;
					lpSoundBuffer->fo_value = 0;
				}
				lpSoundBuffer->nextArcFileNum = Pack_bgmfile;
				lpSoundBuffer->nextMusicNum = num;
				lpSoundBuffer->nextMusicVolume = volume;
				lpSoundBuffer->nextRepeat = repeat;
				if(status!=PCM_FADEOUT)FadeOut(handle, fo_frame);
				goto SetMusicInfo;
			}
		}
		ret = CreateStreamBuffer(handle,Pack_bgmfile,num);
		if(-1==ret){
			bChangeingMusic = FALSE;
			return Cl_PCM_NOTFOUND;
		}
	}else{
		CreateSoundObj(handle,Pack_bgmfile,num,TRUE);
		if(-1==handle){
			bChangeingMusic = FALSE;
			return Cl_PCM_NOTFOUND;
		}
	}
	SetVolume(handle, volume, bgmVolume);
...
ClResult ClSoundDS::playVoiceNum(int &handle,int arcFileNum,int charNo,BOOL bLoop,int volume,int voice_set)
{
	char		workBuf[32];
	ClResult	ret;
	if((-1)==voice_set){
		wsprintf(workBuf,"%05d_%03d_%03d",scriptNo,voiceCnt,charNo);
	}else{
		wsprintf(workBuf,"%05d_%03d_%03d",scriptNo,voice_set,charNo);
	}
	ret = playVoice(handle,arcFileNum,workBuf,bLoop,volume);
	return ret;
} // ClSoundDS::playVoiceNum

ClResult ClSoundDS::playVoice(int &handle,int arcFileNum,char *fname,BOOL bLoop,int volume)
{
	char	workBuf[64];
	BOOL			ret;
	ClSoundBuffer	*lpSoundBuffer;
	...
	if(num==-1){
		myOutputDebugString("Voice %s が見つかりませんでした",fname);
		return Cl_PCM_NOTFOUND;
	}
	myOutputDebugString("Voice:%s",fname);
	if(0==voiceVolume)return Cl_OK;	
	lpSoundBuffer = SelectSoundBuffer(handle);
	if(lpSoundBuffer){		
		int status = GetStatus(handle);
		if(status != PCM_STOP){
	...
	SetVolume(handle,volume,voiceVolume);
	if(FALSE==bLoop){
		play(handle,1);
	}else{
		play(handle,0);	
...
void ClSoundDS::SetVoiceNo(int cnt, int scrNo)
{
	if( (-1)==cnt){
		voiceCnt ++;
	}else{
...
ClResult ClSoundDS::play(int handle,int repeat)
{
	...
	if(memory_sound==lpSoundBuffer->bufType){
		...
		SetThreadPriority(lpSoundBuffer->hLoopThread, THREAD_PRIORITY_HIGHEST);
		lpSoundBuffer->lpDSBuffer->Play(0,0,DSBPLAY_LOOPING);
	}
...
void ClSoundDS::NextMusicStart()
{
	...
	while(lpNext){
		...
		if(lpSoundBuffer && lpSoundBuffer->nextMusicNum>=0){
			...
			int volume = lpSoundBuffer->nextMusicVolume;
...
ClResult ClSoundDS::SetVolumeFade(int handle,int value, int frame)
{
	ClSoundBuffer	*lpSoundBuffer;
	DWORD			dwThreadID;
	...
	if(handle==bgmHandle){
		if(0==value)musicNum = -1;
		musicVol = value;
	}
	if(lpSoundBuffer->status==PCM_PLAY || lpSoundBuffer->status==PCM_FADEOUT){	
		if(lpSoundBuffer->status==PCM_FADEOUT){
			LONG	db = DSBVOLUME_MIN;
			BYTE level;
			if(bgmHandle==lpSoundBuffer->handle) level = bgmVolume;
			else if(voiceHandle==lpSoundBuffer->handle) level = voiceVolume;
			else level = seVolume;
			float f_volume = lpSoundBuffer->fo_value
							+float(lpSoundBuffer->volume -lpSoundBuffer->fo_value) *lpSoundBuffer->fo_step /lpSoundBuffer->fo_frame;
			lpSoundBuffer->volume = int(f_volume);
			lpSoundBuffer->repeat = 0;
			lpSoundBuffer->fo_frame = frame;
			lpSoundBuffer->fo_step = lpSoundBuffer->fo_frame -1;
			lpSoundBuffer->fo_value = value;
	...
	return Cl_PLAY_CONTINUE;
} // ClSoundDS::SetVolumeFade

ClResult ClSoundDS::FadeOut(int handle,int frame)
{
	return(SetVolumeFade(handle, 0, frame));
} // ClSoundDS::FadeOut
...
BYTE ClSoundDS::GetVolume(int handle)
{
	ClSoundBuffer	*lpSoundBuffer;

	lpSoundBuffer = SelectSoundBuffer(handle);
	if(NULL==lpSoundBuffer) return 0;	
	return lpSoundBuffer->volume;
}

void ClSoundDS::SetVolume(int handle,int value,int level)
{
	ClSoundBuffer	*lpSoundBuffer;
	LONG			db = DSBVOLUME_MIN;
	...
	lpSoundBuffer = SelectSoundBuffer(handle);
	if(NULL==lpSoundBuffer) return;	
	lpSoundBuffer->volume = value;
	if(value>0 && level>0){
		db = LONG(log10(value*level/255.0f/10) *20*100);
	}
	lpSoundBuffer->lpDSBuffer->SetVolume(db);
} // ClSoundDS::SetVolume

void ClSoundDS::SetVolumeLevel(int handle,int level)
{
	ClSoundBuffer	*lpSoundBuffer;
	LONG			db = DSBVOLUME_MIN;
	...
	lpSoundBuffer = SelectSoundBuffer(handle);
	if(NULL==lpSoundBuffer) return;	
	if(level>0 && lpSoundBuffer->volume>0){
		db = LONG(log10(lpSoundBuffer->volume*level/255.0f/10) *20*100);
	}
	lpSoundBuffer->lpDSBuffer->SetVolume(db);
} // ClSoundDS::SetVolumeLevel

int ClSoundDS::GetRepeatNum(int handle)
{
	ClSoundBuffer	*lpSoundBuffer;
	lpSoundBuffer = SelectSoundBuffer(handle);
	if(NULL==lpSoundBuffer) return 1;	
	return lpSoundBuffer->repeat;
} // ClSoundDS::GetRepeatNum
...
BOOL ClSoundDS::AddSoundBuffer(int handle,char *fileBuf, int sndNum, BOOL b3DSound)
{
	HRESULT			hr;
	RiffHead		riffHead;
	FmtHead			fmtHead;
	...
	lpDSBufferTail->musicNum = lpDSBufferTail->orgMusicNum = sndNum;
...
DWORD WINAPI fo_exec(LPVOID param)
{
	...
	if(lpSoundBuffer->nextMusicNum>=0){
		my_delete(lpSoundDS->lpNextMusic);
	}
	while(lpSoundBuffer->fo_step>=0){
		if(PCM_FADEOUT!=lpSoundBuffer->status)break;	
		LONG	db = DSBVOLUME_MIN;
		if(bgmHandle==lpSoundBuffer->handle) volume = lpSoundDS->bgmVolume;
		else if(voiceHandle==lpSoundBuffer->handle) volume = lpSoundDS->voiceVolume;
		else volume = lpSoundDS->seVolume;
		float f_volume = lpSoundBuffer->fo_value
						+float(lpSoundBuffer->volume -lpSoundBuffer->fo_value) *lpSoundBuffer->fo_step /lpSoundBuffer->fo_frame;
		if(volume*f_volume > 0){
			db = long(log10(f_volume*volume/255.0f/10.0) *20*100);
			lpSoundBuffer->lpDSBuffer->SetVolume(db);
		}else{
			lpSoundBuffer->lpDSBuffer->SetVolume(DSBVOLUME_MIN);
		}
		if(0>=lpSoundBuffer->fo_step){
			lpSoundBuffer->status = PCM_PLAY;
			lpSoundBuffer->volume = lpSoundBuffer->fo_value;
			lpSoundBuffer->bFade = FALSE;
			ExitThread(TRUE);
			return 0L;
		}
		lpSoundBuffer->fo_step --;
		Sleep(fo_fps);
		if(0>=lpSoundBuffer->fo_step && 0>=lpSoundBuffer->fo_value)break;	
	}
